...
@JvmOverloads constructor(
    ...
    fun setOnErrorClickListener(listener: OnClickListener) {
        getView(ViewState.ERROR)?.findViewById<TextView>(R.id.textViewMessage)
            ?.setOnClickListener(listener)
    }

    fun setOnEmptyClickListener(listener: OnClickListener) {
        getView(ViewState.EMPTY)?.findViewById<TextView>(R.id.textViewMessage)
            ?.setOnClickListener(listener)
    }
    ...
    /**
     * Animates the layout_dialog_scenic_order changes between [ViewState]
     *
